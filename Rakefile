...
namespace :build do
  ...
  task :development => [:clean] do
    sh "jekyll build --drafts --trace"
  end

  desc "build production site"
  task :production => [:clean] do
    sh "JEKYLL_ENV=production jekyll build --config=_config.yml --trace"
  end
end
...
namespace :serve do
  ...
  task :development => [:clean] do
    sh "jekyll serve --drafts --trace"
  end

  desc "serve production site"
  task :production => [:clean] do
    sh "JEKYLL_ENV=production jekyll serve --config=_config.yml --trace"
  end
end
